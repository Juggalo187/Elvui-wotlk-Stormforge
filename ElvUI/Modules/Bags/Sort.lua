...
function B.SortBags(...)
	...
	if #args > 0 and type(args[1]) == "boolean" then
		...
		startIndex = 2
		E:Print("SortBags: Using Shift override, reverse = " .. tostring(actualReverse))
	else
		E:Print("SortBags: Using setting from options: " .. tostring(actualReverse))
	end
	
	E:Print("SortBags: final actualReverse = " .. tostring(actualReverse))
	E:Print("SortBags: startIndex = " .. startIndex)
	E:Print("SortBags: #args = " .. #args)

	-- Process the bag groups starting from startIndex
	for i = startIndex, #args do
		local bags = args[i]
		E:Print("SortBags: processing bags at index " .. i .. ", type = " .. type(bags))
		if type(bags) == "table" then
			...
			E:Print("SortBags: WARNING - expected table at index " .. i .. ", got " .. type(bags))
		end
	end
end
...
function B:CommandDecorator(func, groupsDefaults)
	return function(reverse)
		...
		E:Print("CommandDecorator: reverse = " .. tostring(reverse))
		E:Print("CommandDecorator: IsShiftKeyDown() = " .. tostring(IsShiftKeyDown()))
		...
		if reverse ~= nil then
			E:Print("Calling func with reverse = " .. tostring(reverse) .. " (Shift override)")
			func(reverse, unpack(bagGroups))
		else
			E:Print("Calling func with no reverse parameter (using settings)")
